...
// ── MIDI-LLM sidecars (plan 2026-08-28) — type-aware clustering/related ─────

namespace {

// The 20 kMidiFeatureKeys in fixed order (mirrors LibraryClusterer.h).
const char* kMidiKeys[20] = {
...
QString midiSidecarJson(const char* words, const char* prose, const char* flavor) {
    QStringList pairs;
    for (int i = 0; i < 20; ++i)
        pairs << QString("\"%1\": %2").arg(QString::fromUtf8(kMidiKeys[i]))
                                       .arg(QString::number(0.05 + 0.05 * i, 'f', 6));
    return QStringLiteral("{\"dsp_words\":\"%1\",\"prose\":\"%2\","
        "\"captions\":[[\"%3 pattern\",1.0]],\"tags\":[[\"%3\",1.0]],"
        "\"dsp\":{%4}}")
        .arg(QString::fromUtf8(words), QString::fromUtf8(prose),
             QString::fromUtf8(flavor), pairs.join(","));
}

bool writeMidiWithSidecar(const QString& dir, const char* name,
                          const char* words, const char* prose, const char* flavor) {
    const std::string base = dir.toStdString() + "/" + name;
    {
        ...
        mf.addTrack(seq);
        juce::FileOutputStream s{ juce::File(base) };
        mf.writeTo(s);
        s.flush();
    }
    juce::File sidecar(base + ".mid.json");
    sidecar.replaceWithText(midiSidecarJson(words, prose, flavor).toStdString());
    return true;
}

} // namespace

TEST_F(McpCoverageTest, LibraryClusterMidiTwoSidecarFamilies) {
    QTemporaryDir dir;
    ASSERT_TRUE(dir.isValid());
    ASSERT_TRUE(writeMidiWithSidecar(dir.path(), "dark1.mid", "dark techno bassline",
                                     "a driving dark techno bassline", "dark"));
    ASSERT_TRUE(writeMidiWithSidecar(dir.path(), "dark2.mid", "dark techno bassline",
                                     "a driving dark techno bassline", "dark"));
    ASSERT_TRUE(writeMidiWithSidecar(dir.path(), "airy1.mid", "ambient airy pad",
                                     "a slow ambient airy pad", "airy"));
    ASSERT_TRUE(writeMidiWithSidecar(dir.path(), "airy2.mid", "ambient airy pad",
                                     "a slow ambient airy pad", "airy"));

    auto addR = call("add_library", {{"name", "MidiClusterLib"}, {"path", dir.path()}, {"type", "midi"}});
    ASSERT_FALSE(isError(addR)) << text(addR).toStdString();
    QString libId = QJsonDocument::fromJson(text(addR).toUtf8()).object().value("id").toString();
    ASSERT_FALSE(libId.isEmpty());
    ...
    // Search over the midi library matches sidecar words (tags/prose).
    auto searchR = call("search_library", {{"query", "bassline"}, {"type", "midi"}});
    ASSERT_FALSE(isError(searchR)) << text(searchR).toStdString();
    auto searchArr = QJsonDocument::fromJson(text(searchR).toUtf8()).array();
    ASSERT_EQ(searchArr.size(), 2) << "both dark family files match sidecar words";

    QJsonArray ids{libId};

    // cluster_library with type "midi" -> two families, members carry tags.
    auto clusterR = call("cluster_library", {{"libraryIds", ids}, {"k", 2}, {"type", "midi"}});
    ASSERT_FALSE(isError(clusterR)) << text(clusterR).toStdString();
    auto obj = QJsonDocument::fromJson(text(clusterR).toUtf8()).object();
    EXPECT_EQ(obj.value("method").toString().toStdString(), "hybrid");
    EXPECT_EQ(obj.value("k").toInt(), 2);
    auto clusters = obj.value("clusters").toArray();
    ASSERT_EQ(clusters.size(), 2) << "two midi sidecar families -> two clusters";
    int dark = 0, airy = 0;
    for (const auto& c : clusters) {
        auto co = c.toObject();
        EXPECT_FALSE(co.value("label").toString().isEmpty());
        ...
        for (const auto& m : co.value("members").toArray()) {
            ...
            EXPECT_TRUE(mo.contains("tags"));
            EXPECT_TRUE(mo.value("tags").toString().contains("dark")
                        || mo.value("tags").toString().contains("airy"));
            EXPECT_TRUE(mo.contains("similarity"));
            if (mo.value("name").toString().startsWith("dark")) ++dark; else ++airy;
        }
    }
    EXPECT_EQ(dark, 2);
    EXPECT_EQ(airy, 2);

    // Wrong declared type on the same midi ids -> tool error, never a result.
    auto wrongType = call("cluster_library", {{"libraryIds", ids}, {"k", 2}, {"type", "audio"}});
    EXPECT_TRUE(isError(wrongType));
    EXPECT_TRUE(text(wrongType).contains("not audio libraries"));

    // Unknown type value -> schema enums reject it pre-handler ("value not
    // in enum"); the engine-side "unknown type" error path is covered at the
    // manager level (FileLibraryTest.ClusterLibraryMidiTypeScopeAndErrors).
    auto badType = call("cluster_library", {{"libraryIds", ids}, {"k", 2}, {"type", "video"}});
    EXPECT_TRUE(isError(badType));

    // related_samples with a midi seed (path from the indexed entries):
    // the same-family neighbor ranks first, seed excluded.
    QString dark1Path;
    for (const auto& e : searchArr)
        if (e.toObject().value("name").toString() == "dark1.mid")
            dark1Path = e.toObject().value("path").toString();
    ASSERT_FALSE(dark1Path.isEmpty());
    auto relR = call("related_samples", {{"libraryIds", ids}, {"filePath", dark1Path}, {"type", "midi"}});
    ASSERT_FALSE(isError(relR)) << text(relR).toStdString();
    auto relObj = QJsonDocument::fromJson(text(relR).toUtf8()).object();
    EXPECT_EQ(relObj.value("seed").toObject().value("name").toString().toStdString(), "dark1.mid");
    auto results = relObj.value("results").toArray();
    ASSERT_GE(results.size(), 1);
    EXPECT_TRUE(results[0].toObject().value("name").toString().startsWith("dark"));

    // Cleanup so the appdata registry is not polluted (mirrors the audio tests).
    auto removeR = call("remove_library", {{"id", libId}});
    EXPECT_FALSE(isError(removeR)) << text(removeR).toStdString();
}
