...
TEST_F(FileLibraryTest, PartitioningByFirstChar) {
    ...
    // Partition 'a': alpha.mid
    {
        ...
        root->setProperty("schemaVersion", 3); // midi caches are v3 (per-type contract, 2026-08-28)
...
// ── MIDI-LLM sidecars (plan 2026-08-28) ──────────────────────────────────────

namespace {

// <file>.mid.json sidecar with the 20 kMidiFeatureKeys values
// (valueScale * (0.5 + 0.1*i)). dropKey < 0 writes all keys; else that key
// is omitted (all-20-finite rule). captionsJson/tagsJson default to the
// standard triple when empty.
juce::String midiSidecarJson(const juce::String& dspWords, const juce::String& prose,
                             const juce::String& captionsJson, const juce::String& tagsJson,
                             int dropKey, double valueScale) {
    juce::StringArray pairs;
    for (int i = 0; i < HDAW::kMidiFeatureCount; ++i) {
        if (i == dropKey) continue;
        pairs.add("\"" + juce::String(HDAW::kMidiFeatureKeys[i]) + "\": "
                  + juce::String(valueScale * (0.5 + 0.1 * (double)i), 8));
    }
    return "{\n"
           "  \"dsp_words\": \"" + dspWords + "\",\n"
           "  \"prose\": \"" + prose + "\",\n"
           "  \"captions\": " + (captionsJson.isEmpty()
               ? juce::String("[[\"minor key pattern\", 1.0], [\"offbeat groove\", 0.9], [\"bass-focused\", 0.8]]")
               : captionsJson) + ",\n"
           "  \"tags\": " + (tagsJson.isEmpty()
               ? juce::String("[[\"techno\", 0.9], [\"minor\", 0.8], [\"bassline\", 0.7]]")
               : tagsJson) + ",\n"
           "  \"dsp\": { " + pairs.joinIntoString(", ") + " }\n}\n";
}

void writeMidiSidecar(const juce::File& midiFile, const juce::String& dspWords,
                      const juce::String& prose, const juce::String& captionsJson = {},
                      const juce::String& tagsJson = {}, int dropKey = -1,
                      double valueScale = 1.0) {
    juce::File(midiFile.getFullPathName() + ".mid.json")
        .replaceWithText(midiSidecarJson(dspWords, prose, captionsJson, tagsJson, dropKey, valueScale));
}

// Proven pattern from ExtractMidiMetadata: a small MIDI file written
// programmatically at the given path.
void writeSimpleMidi(const juce::File& f) {
    juce::MidiMessageSequence seq;
    seq.addEvent(juce::MidiMessage::noteOn(1, 60, (juce::uint8)80).withTimeStamp(0.0));
    seq.addEvent(juce::MidiMessage::noteOff(1, 60).withTimeStamp(1.0));
    seq.addEvent(juce::MidiMessage::noteOn(1, 64, (juce::uint8)70).withTimeStamp(1.0));
    seq.addEvent(juce::MidiMessage::noteOff(1, 64).withTimeStamp(2.0));
    juce::MidiFile file;
    file.setTicksPerQuarterNote(480);
    file.addTrack(seq);
...
TEST_F(FileLibraryTest, MidiSidecarIngestionPopulatesTagsAndDescription) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_ingest");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    writeMidiSidecar(midiFile, "minor-key bassline, offbeat",
                     "A driving minor-key bassline with offbeat 16th syncopation, ideal for techno.");

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Sidecar", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);
    ASSERT_FALSE(mgr.isScanning()) << "scan did not complete in time";
    ...
    auto results = mgr.search("");
    ASSERT_EQ(results.size(), 1u);
    EXPECT_EQ(results[0].name, "pattern.mid");
    // tags = dsp_words + top-3 captions + top-3 tags, comma-joined
    EXPECT_TRUE(results[0].tags.startsWith("minor-key bassline, offbeat"));
    EXPECT_TRUE(results[0].tags.contains("minor key pattern"));
    EXPECT_TRUE(results[0].tags.contains("techno"));
    EXPECT_EQ(results[0].description,
              "A driving minor-key bassline with offbeat 16th syncopation, ideal for techno.");

    // Text search parity: matches via tags (dsp_words) and via prose.
    auto byTag = mgr.search("offbeat");
    ASSERT_EQ(byTag.size(), 1u);
    EXPECT_EQ(byTag[0].name, "pattern.mid");
    auto byProse = mgr.search("syncopation");
    ASSERT_EQ(byProse.size(), 1u);
}

TEST_F(FileLibraryTest, MidiSidecarFeaturesIngestedWhenAllTwentyKeysPresent) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_feats");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    writeMidiSidecar(midiFile, "techno bassline", "text");

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Feat", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);

    auto results = mgr.search("");
    ASSERT_EQ(results.size(), 1u);
    ASSERT_EQ(results[0].dspFeatures.size(), (size_t)HDAW::kMidiFeatureCount);
    EXPECT_NEAR(results[0].dspFeatures[0], 0.5, 1e-6);
    EXPECT_NEAR(results[0].dspFeatures[7], 1.2, 1e-6);
    EXPECT_NEAR(results[0].dspFeatures[19], 2.4, 1e-6);
}

TEST_F(FileLibraryTest, MidiSidecarMissingKeyLeavesFeaturesEmpty) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_missing");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    writeMidiSidecar(midiFile, "partial analysis", "text", {}, {}, 7); // drop pitch_centroid

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Missing", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);

    auto results = mgr.search("");
    ASSERT_EQ(results.size(), 1u);
    EXPECT_TRUE(results[0].dspFeatures.empty()) << "all-20-finite rule: 19 keys must stay empty";
    EXPECT_TRUE(results[0].tags.startsWith("partial analysis")) << "text axis still ingests";
}

TEST_F(FileLibraryTest, MidiSidecarMtimeBumpTriggersRescan) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_mtime");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Mtime", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);
    auto before = mgr.search("");
    ASSERT_EQ(before.size(), 1u);
    ...
    // Sidecar appears after the first scan -> its mtime is newer (mirrors the
    // audio sidecar trap: a copied pack with an older sidecar is NOT re-run).
    juce::Thread::sleep(10);
    writeMidiSidecar(midiFile, "late analysis", "ingested on the second scan");
    mgr.scanLibrary(id);
    waitForScan(mgr);
    auto after = mgr.search("");
    ASSERT_EQ(after.size(), 1u);
    EXPECT_TRUE(after[0].tags.startsWith("late analysis"));
}

TEST_F(FileLibraryTest, MidiSidecarRoundTripAcrossInstances) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_roundtrip");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    writeMidiSidecar(midiFile, "round trip tags", "round trip prose");

    {
        HDAW::FileLibraryManager mgr(tempDir);
        auto id = mgr.addLibrary("Midi RT", midiDir.getFullPathName(), "midi");
        mgr.scanLibrary(id);
        waitForScan(mgr);
    }

    // New manager on the same base dir, NO scan: lazy-load must restore the
    // sidecar data from the v3 midi cache.
    HDAW::FileLibraryManager mgr2(tempDir);
    auto results = mgr2.search("");
    ASSERT_EQ(results.size(), 1u);
    EXPECT_TRUE(results[0].tags.startsWith("round trip tags"));
    EXPECT_EQ(results[0].description, "round trip prose");
    ASSERT_EQ(results[0].dspFeatures.size(), (size_t)HDAW::kMidiFeatureCount);
}

TEST_F(FileLibraryTest, MidiSidecarMalformedTolerated) {
    auto midiDir = tempDir.getChildFile("midi_sidecar_malformed");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    juce::File(midiFile.getFullPathName() + ".mid.json").replaceWithText("{ this is not json !!");

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Mal", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);
    auto results = mgr.search("");
    ASSERT_EQ(results.size(), 1u);
    EXPECT_TRUE(results[0].tags.isEmpty());
    EXPECT_TRUE(results[0].dspFeatures.empty());
}

TEST_F(FileLibraryTest, MidiCacheSchemaBelow3Ignored) {
    auto midiDir = tempDir.getChildFile("midi_schema_guard");
    midiDir.createDirectory();
    auto midiFile = midiDir.getChildFile("pattern.mid");
    writeSimpleMidi(midiFile);
    writeMidiSidecar(midiFile, "fresh tags", "fresh prose");

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Schema", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);

    // Forge a v2 (pre-midi-features) cache with an entry lacking the sidecar
    // data. The midi-library load guard must reject it wholesale.
    {
        juce::DynamicObject::Ptr root = new juce::DynamicObject();
        juce::Array<juce::var> arr;
        juce::DynamicObject::Ptr e = new juce::DynamicObject();
        e->setProperty("name", "pattern.mid");
        e->setProperty("path", midiFile.getFullPathName());
        e->setProperty("size", (double)midiFile.getSize());
        e->setProperty("modifiedTime", (double)midiFile.getLastModificationTime().toMilliseconds());
        e->setProperty("tracks", 1);
        e->setProperty("notes", 2);
        e->setProperty("format", "midi");
        arr.add(juce::var(e.get()));
        root->setProperty("entries", arr);
        root->setProperty("schemaVersion", 2); // pre-v3 midi cache
        tempDir.getChildFile("libraries").getChildFile(id + ".json")
            .replaceWithText(juce::JSON::toString(juce::var(root.get())));
    }

    // The v2 cache must be ignored: search sees nothing until a rescan
    // re-ingests from disk + sidecar.
    HDAW::FileLibraryManager mgr2(tempDir);
    EXPECT_EQ(mgr2.search("").size(), 0u);

    mgr2.scanLibrary(id);
    waitForScan(mgr2);
    auto res = mgr2.search("");
    ASSERT_EQ(res.size(), 1u);
    EXPECT_TRUE(res[0].tags.startsWith("fresh tags"));
    ASSERT_EQ(res[0].dspFeatures.size(), (size_t)HDAW::kMidiFeatureCount);
}

TEST_F(FileLibraryTest, ClusterLibraryMidiTypeScopeAndErrors) {
    auto midiDir = tempDir.getChildFile("midi_cluster_scope");
    midiDir.createDirectory();
    auto a1 = midiDir.getChildFile("alpha1.mid");
    auto a2 = midiDir.getChildFile("alpha2.mid");
    auto b1 = midiDir.getChildFile("beta1.mid");
    auto b2 = midiDir.getChildFile("beta2.mid");
    for (auto* f : {&a1, &a2, &b1, &b2}) writeSimpleMidi(*f);
    writeMidiSidecar(a1, "dark techno bassline", "a driving dark techno bassline",
                     "[[\"techno\",1.0],[\"bassline\",0.9],[\"driving\",0.8]]",
                     "[[\"techno\",1.0],[\"bassline\",0.9],[\"dark\",0.8]]");
    writeMidiSidecar(a2, "dark techno bassline", "a driving dark techno bassline",
                     "[[\"techno\",1.0],[\"bassline\",0.9],[\"driving\",0.8]]",
                     "[[\"techno\",1.0],[\"bassline\",0.9],[\"dark\",0.8]]");
    writeMidiSidecar(b1, "ambient airy pad", "a slow ambient airy pad",
                     "[[\"ambient\",1.0],[\"pad\",0.9],[\"slow\",0.8]]",
                     "[[\"ambient\",1.0],[\"pad\",0.9],[\"airy\",0.8]]");
    writeMidiSidecar(b2, "ambient airy pad", "a slow ambient airy pad",
                     "[[\"ambient\",1.0],[\"pad\",0.9],[\"slow\",0.8]]",
                     "[[\"ambient\",1.0],[\"pad\",0.9],[\"airy\",0.8]]");

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Midi Cluster", midiDir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);

    juce::String error;
    // type "midi" -> the two sidecar families cluster separately.
    auto outcome = mgr.clusterLibrary(juce::StringArray{id}, 2, "hybrid", error,
                                      {}, {}, nullptr, "midi");
    EXPECT_TRUE(error.isEmpty()) << error;
    ASSERT_EQ(outcome.clusters.size(), 2u);
    int alphaMembers = 0, betaMembers = 0;
    for (const auto& c : outcome.clusters)
        for (const auto& m : c.members)
            if (m.name.startsWith("alpha")) ++alphaMembers; else ++betaMembers;
    EXPECT_EQ(alphaMembers, 2);
    EXPECT_EQ(betaMembers, 2);

    // Same ids with the wrong declared type -> error, never a result.
    error = {};
    auto wrong = mgr.clusterLibrary(juce::StringArray{id}, 2, "hybrid", error,
                                    {}, {}, nullptr, "audio");
    EXPECT_TRUE(wrong.clusters.empty());
    EXPECT_TRUE(error.contains("not audio libraries")) << error;

    // Omitted ids + audio -> no audio libraries.
    error = {};
    auto noAudio = mgr.clusterLibrary(juce::StringArray(), 2, "hybrid", error, {}, {}, nullptr, "audio");
    EXPECT_TRUE(noAudio.clusters.empty());
    EXPECT_TRUE(error.contains("no audio libraries")) << error;

    // Omitted ids + midi -> all-midi scope selects the midi library.
    error = {};
    auto allMidi = mgr.clusterLibrary(juce::StringArray(), 2, "hybrid", error, {}, {}, nullptr, "midi");
    EXPECT_TRUE(error.isEmpty()) << error;
    EXPECT_EQ(allMidi.clusters.size(), 2u);

    // Unknown type value -> validation error.
    error = {};
    auto badType = mgr.clusterLibrary(juce::StringArray{id}, 2, "hybrid", error,
                                      {}, {}, nullptr, "video");
    EXPECT_TRUE(badType.clusters.empty());
    EXPECT_TRUE(error.contains("unknown type")) << error;

    // relatedSamples over a midi seed: same-family neighbor first, seed excluded.
    error = {};
    auto rel = mgr.relatedSamples(juce::StringArray{id}, a1.getFullPathName(), {},
                                  5, "hybrid", error, "midi");
    EXPECT_TRUE(error.isEmpty()) << error;
    ASSERT_EQ(rel.results.size(), 3u); // the other three files only
    EXPECT_TRUE(rel.results[0].name.startsWith("alpha"));
}

TEST_F(FileLibraryTest, RealMidiCorpusEndToEnd) {
    // Opt-in real-corpus smoke (G5 companion): HDAW_MIDI_CORPUS=<dir with
    // .mid files + <file>.mid.json sidecars> runs the genuine ingestion ->
    // cluster -> related path over a real corpus (e.g. the repo's teknoir/
    // once midi_analyze.py has written sidecars). Skipped when unset so the
    // suite stays hermetic.
    const char* corpus = std::getenv("HDAW_MIDI_CORPUS");
    if (corpus == nullptr || corpus[0] == '\0')
        GTEST_SKIP() << "HDAW_MIDI_CORPUS unset — real-corpus smoke skipped";
    // Trim defensively: cmd `set VAR=x && ...` preserves the space before
    // `&&`, and a trailing space breaks FindFirstFile on the directory.
    juce::String corpusStr = juce::String::fromUTF8(corpus).trim();
    const juce::File dir(corpusStr);
    ASSERT_TRUE(dir.isDirectory()) << corpus;

    HDAW::FileLibraryManager mgr(tempDir);
    auto id = mgr.addLibrary("Corpus", dir.getFullPathName(), "midi");
    mgr.scanLibrary(id);
    waitForScan(mgr);

    auto all = mgr.search("");
    ASSERT_GT(all.size(), 5u) << "corpus must hold more than 5 midi entries";
    int withTags = 0, withFeats = 0;
    for (const auto& e : all) {
        if (!e.tags.isEmpty()) ++withTags;
        if (!e.dspFeatures.empty()) ++withFeats;
    }
    EXPECT_GT(withTags, (int)all.size() * 3 / 4) << withTags << "/" << all.size() << " entries carry sidecar tags";
    EXPECT_GT(withFeats, (int)all.size() * 3 / 4) << withFeats << "/" << all.size() << " entries carry 20-dim features";

    // Auto-k clustering over the whole corpus on the numeric axis only.
    juce::String error;
    auto out = mgr.clusterLibrary(juce::StringArray(), 0, "dsp", error, {}, {}, nullptr, "midi");
    EXPECT_TRUE(error.isEmpty()) << error;
    EXPECT_GT(out.clusters.size(), 0u);

    // Related over the first entry: same-family ranking, exactly 5 results.
    auto rel = mgr.relatedSamples(juce::StringArray{id}, all[0].path, {}, 5, "hybrid", error, "midi");
    EXPECT_TRUE(error.isEmpty()) << error;
    EXPECT_EQ(rel.results.size(), 5u);
}
