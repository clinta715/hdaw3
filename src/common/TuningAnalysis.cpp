...
QMap<QString, RoleTarget> roleTargets()
{
...
QString normalizeRole(QString r) {
    r = r.trimmed().toLower();
...
// hz<->mel (kept from the pre-extraction file; the C++ fallback does not use them yet —
// the Python sidecar owns exact band math)
...
// median helper (unused by the fallback today; part of the extracted surface)
...
Descriptors computeDescriptors(const std::vector<float>& x, double sr)
{
    Descriptors d;
    d.sampleRate = sr;
...
QString suggestFor(const QString& role, const Descriptors& desc, const RoleTarget& tgt)
{
...
QJsonObject checkRole(const QString& roleIn, const Descriptors& d)
{
...
TuningAnalysisResult runAnalysis(const QString& wavPath, const QString& role)
{
    if (wavPath.isEmpty())
    ...
    TuningAnalysisResult res;
    res.object = out;
    res.rawJson = QString::fromUtf8(QJsonDocument(out).toJson(QJsonDocument::Indented));
    return res;
}
...
{
    return runAnalysis(wavPath, role);
}
