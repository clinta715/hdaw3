...
s.registerTool({"psy_fm_mod_matrix_debug",
        ...
        [e](const QJsonObject& a) -> McpToolResult {
            ...
            // Canonical payload: src/common/PsyFmModMatrixView.cpp, shared with the
            // frontend RPC method psy_fm.modMatrixDebug so the two surfaces cannot
            // drift (AGENTS.md "RPC parity by construction").
            const auto out = HDAW::buildPsyFmModMatrixView(
                e->getProjectModel(), e->getMainProcessor(), ti, si);
            if (out.isEmpty())
