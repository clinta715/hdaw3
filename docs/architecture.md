...
**Current scope**: HDAW is a JUCE 8 desktop DAW at version **0.25.0**
...
- **Frontend build (DEPRECATED 2026-09-23 — engine work never builds the frontend):** the React frontend is built separately (`cd frontend; npm run build`) and its output (`dist/index.html`, `dist/assets/index.js`, `dist/assets/index.css`) is compiled into `HDAW.exe` via Qt resources (`src/resources/frontend.qrc`). The Electron client is a separate project now — see AGENTS.md "DEPRECATED (2026-09-23)".

## Version Management

Version numbers are stored in **two places** and must be kept in sync manually:
- `CMakeLists.txt` → `project(HDAW VERSION 0.25.0 ...)` — **canonical source** for C++ code
- `frontend/package.json` → `"version": "0.25.0"` — **canonical source** for the React frontend
...
**Frontend code (DEPRECATED 2026-09-23 — reference only, the client is not a delivery target)** reads the version from `frontend/src/version.ts`, auto-generated by the `prebuild` npm script from `package.json`. The `check_version` target above still enforces the two-file match as tooling hygiene.
...
1. *(DEPRECATED 2026-09-23 — the Electron client is a separate project; engine work skips this step)* `cd frontend; npm run build` → Vite produces `dist/index.html`, `dist/assets/index.js`, `dist/assets/index.css`
